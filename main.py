...
def _run_lightweight_migrations():
    statements = [
        ...
        # "Originating Revenue Source" — the department/source a revenue
        # sub-item is generated by (e.g. "Finance", "Works & Technical").
...
class RevenueLineItemIn(BaseModel):
    ...
    code: Optional[str] = None
    label: str
    originating_revenue_source: Optional[str] = None
    row_type: str = "item"
    indent: int = 0
...
def revenue_line_item_to_out(r: RevenueLineItem) -> RevenueLineItemOut:
    ...
    return RevenueLineItemOut(
        ...
        code=r.code,
        label=r.label,
        originating_revenue_source=r.originating_revenue_source,
        row_type=r.row_type,
...
def _parse_quarterly_style_sheet(ws, has_originating_source: bool = False) -> List[dict]:
    """Shared by the "quarterly", "rev_by_source" and "rev_by_category" pages.
    "quarterly" and "rev_by_source" use: A=Code, B=Fund Category and Source
    of Financing, C:F (idx 2-5)=Q1-Q4 Approved, G (idx 6)=Annual Approved,
    H:K (idx 7-10)=Q1-Q4 Actual, L (idx 11)=Annual Actual.

    "rev_by_category" (has_originating_source=True) inserts one extra text
    column, "Originating Revenue Source", right after the label, shifting
    everything after it by one: A=Code, B=Category & Source,
    C=Originating Revenue Source, D:G=Q1-Q4 Approved, H=Annual Approved,
    I:L=Q1-Q4 Actual, M=Annual Actual.

    Any Gap columns present in the workbook are ignored either way — the
    gap is always recomputed as Approved − Actual on read."""
    out = []
    all_rows = list(ws.iter_rows(values_only=True))
    start = 3 if has_originating_source else 2
    for row in all_rows[2:]:
        if row is None or all(c is None or str(c).strip() == "" for c in row):
            continue
        code = row[0]
        ...
        if has_originating_source:
            raw_source = row[2] if len(row) > 2 else None
            originating_source = str(raw_source).strip() if raw_source is not None and str(raw_source).strip() != "" else None
        approved = [parse_amount(row[i]) if i < len(row) else 0.0 for i in range(start, start + 4)]
        approved_annual = parse_amount(row[start + 4]) if len(row) > start + 4 else sum(approved)
        actual = [parse_amount(row[i]) if i < len(row) else 0.0 for i in range(start + 5, start + 9)]
        actual_annual = parse_amount(row[start + 9]) if len(row) > start + 9 else sum(actual)
        row_type = _classify_pbs_row(code, label, approved, actual)
        out.append({
            "code": str(code).strip() if code is not None and str(code).strip() != "" else None,
            "label": str(label).strip(),
...
def create_revenue_line_item(payload: RevenueLineItemIn, db: Session = Depends(get_db),
    ...
    r = RevenueLineItem(
        ...
        originating_revenue_source=(payload.originating_revenue_source or "").strip() or None,
...
def update_revenue_line_item(item_id: int, payload: RevenueLineItemUpdate, db: Session = Depends(get_db),
    ...
    if "originating_revenue_source" in data:
        r.originating_revenue_source = (data.pop("originating_revenue_source") or "").strip() or None
    if "row_type" in data:
...
async def import_revenue_line_items(work_plan_id: int, period_type: str, file: UploadFile = File(...),
    ...
    for i, row in enumerate(parsed):
        db.add(RevenueLineItem(
            ...
            code=row["code"],
            label=row["label"],
            originating_revenue_source=row.get("originating_revenue_source"),
